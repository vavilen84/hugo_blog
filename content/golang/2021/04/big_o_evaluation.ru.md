...
Большое "O" (Big O) - определение сложности алгоритма (зависимость производительности от входных данных). личают 
...
```
В результате мы имеем O((N * 1) + (N * 1) + 1) = O(N + N).

Пример ниже имеет сложность O(1 + (N * 1) + (N * 1) + 1 + (N * (N + 1))) = O(N + N + N{{< exp >}}2{{< /exp >}})
```
func algo(n []int) int {
    res := 0 // 1
    for k := range n { N
...
детерминирован (память выделяется только один , затем):
```
func max(n []int) int {
	res := math.MinInt64 // 1
	for k := range n {
...
Алгоритм слияния срезов будет иметь сложность O(N1 + N2), т.к. для результирующего среза будет
выделен объем памяти пропорциональный мерам входных срезов:
```
func merge(a, b []int) []int {
	res := make([]int, len(a)+len(b)) // N1 + N2
	for k := range a {
		res[k] = a[k]
	}
	for k := range b {
...
Алгоритм конкатенации строк будет иметь сложность O(N + K)
```
...
Давайте берем следующий пример: дан массив строк, в нем надо отсортировать вначале сами строки, а, затем, и массив целиком.
```
...
Сложность - O(N), т.к. foo будет вызвана N
```
func foo(n int) {
    if n == 1 {
        return
